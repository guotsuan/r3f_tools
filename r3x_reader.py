...
#                 self.footerFlag = input('0=discard footer\n1=save footer\n> ')
    ...
    def print_header_data(self):
        ...
        print(
            'Reference Time: {0[1]}/{0[2]}/{0[0]} at {0[3]}:{0[4]}:{0[5]}.{0[6]}'.format(self.dFormat.refTime))
        print('Clock sample count: ', self.dFormat.clockSamples[0])
        print('Sample ticks per second: ', self.dFormat.timeSampleRate[0])
        print(
            'UTC Time: {0[1]}/{0[2]}/{0[0]} at {0[3]}:{0[4]}:{0[5]}.{0[6]}'.format(self.dFormat.utcTime))

        print('\nCHANNEL AND SIGNAL PATH CORRECTION')
        print('ADC Scale Factor: ', self.chCorr.adcScale[0])
        print('Signal Path Delay: {} nsec'.format(
            self.chCorr.pathDelay[0]*1e9))
    ...
    def plot_graphs(self):
        ...
        savemat("corr.mat", {'freq':self.chCorr.freqTable,
                            'amptable':self.chCorr.ampTable}, format='5')
        plt.show()
        plt.clf()
    ...
    def file_manager(self):
        ...
        if '.r3f' in self.inFileName:
            ...
            self.fileLength = self.numFrames * \
                (self.dFormat.sampleSize/self.dFormat.sRate)
    ...
    def data_muncher(self):
        # Calls get_adc_samples(), ddc(), and file_saver(),
    ...
    def ddc(self):
        ...
        if self.iqFlag == '1':
            ...
            I = self.ADC*np.squeeze(loI)
            Q = self.ADC*np.squeeze(loQ)
            del(loI)
            del(loQ)
    ...
    def file_saver(self, loop, processData):
        ...
        if self.iqFlag == '1':
            ...
            print('Data file saved at {}'.format(fileName))
